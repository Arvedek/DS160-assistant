...
如果材料都已经放在 `materials/` 文件夹，可以直接点击 `1B. 生成 materials 全量审阅包`。这个包会包含材料清单、文本材料预览、当前草稿字段和 Codex 指令，让 Codex 一次性读取/评估整个 folder，而不是逐个文件处理。
...
If your source files are already in `materials/`, click `1B. Generate materials review package`. The package includes the folder manifest, text previews, current draft context, and Codex instructions so Codex can review the whole case together.
