...
- **Materials Review Package:** 一键把 `materials/` 全部材料整理成 Codex 可执行的 extract/evaluate 任务
- **材料状态表:** 显示每个材料是否可读文本、需要 Codex 读取/附件、是否超出大小限制
- **三种提取方式:** OpenAI API、Codex Handoff、本地文本规则
- **候选字段审阅:** 标记 `fill_empty`、`same_value`、`replace_conflict`，冲突项默认不勾选
- **候选质量评分:** 自动分为 ready、check source、needs review、already confirmed
...
如果材料都已经放在 `materials/` 文件夹，可以直接点击 `1B. 生成 materials 全量审阅包`。这个包会包含材料清单、文本材料预览、当前草稿字段和 Codex 任务单，让 Codex 一次性读取/评估整个 folder，而不是逐个文件处理。Codex 返回结果解析后，本地 app 会给每个候选字段显示质量评分和复核建议。
...
- **Materials Review Package:** Turn the full `materials/` folder into one Codex extract/evaluate task
- **Materials Status Table:** Show text-readable files, files Codex should read/attach, and files that exceed limits
...
- **Candidate quality scoring:** Group results as ready, check source, needs review, or already confirmed
...
If your source files are already in `materials/`, click `1B. Generate materials review package`. The package includes the folder manifest, text previews, current draft context, and a Codex task brief so Codex can review the whole case together. After parsing the Codex result, the local app shows quality tiers and review recommendations for each candidate.
